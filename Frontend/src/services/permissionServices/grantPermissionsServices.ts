...
export const GrantPermissionsServices = {
  ...
  updateGrantPermissions: async (
    ID: string,
    payload: { MaVT: string; oldQuyen: string; newQuyen: string },
  ) => {
    const res = await api.put(
      `/api/permissions/Permission_Role/${ID}`,
      payload,
      {
        withCredentials: true,
      },
  ...
  deleteAllGrantPermissions: async (ID: string) => {
    await api.delete(`/api/permissions/Permission_Role/${ID}`, {
      withCredentials: true,
    });
    return true;
  },
  deleteOneGrantPermissions: async (IDR: string, IDP: string) => {
    const res = await api.delete(
      `/api/permissions/Permission_Role/${IDR}/${IDP}`,
      { withCredentials: true },
    );
    return res.data;
  },
