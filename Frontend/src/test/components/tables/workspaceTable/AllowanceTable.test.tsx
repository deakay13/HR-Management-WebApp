...
vi.mock("@/services/payRollServices/allowanceServices", () => ({
  AllowanceServices: {
    exportAllowance: vi.fn(),
  }
}));
vi.spyOn(i18n, "t").mockImplementation(((key: string) => key) as typeof i18n.t);

// Mock Dialog Component since we only test Table
vi.mock("@/components/table/workspaceTable/dialogs/CreateAllowanceDialog", () => ({
  CreateAllowanceDialog: ({ open, onOpenChange }: any) => {
    return open ? (
      <div data-testid="create-allowance-dialog">
...
vi.mock("@/components/actionCells/workspace/AllowanceActionCell", () => ({
  AllowanceActionCell: () => <div data-testid="allowance-action-cell" />
}));
...
describe("AllowanceTable", () => {
  const mockData = [
    { MaPC: "PC01", MaNV: "NV01", LoaiPC: "Xăng xe", SoTien: 500000 },
    { MaPC: "PC02", MaNV: "NV02", LoaiPC: "Ăn trưa", SoTien: 1000000 },
  ];
  ...
  const renderComponent = (props: any = {}) => render(
    <MemoryRouter>
      <AllowanceTable data={mockData as any} {...props} />
    </MemoryRouter>
  );
  ...
  test("renders data rows", () => {
    ...
    expect(screen.getByText("PC02")).toBeInTheDocument();
  });

  test("can trigger export excel", async () => {
    const mockBlob = new Blob(["test"], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
    (AllowanceServices.exportAllowance as any).mockResolvedValue(mockBlob);
    ...
    expect(AllowanceServices.exportAllowance).toHaveBeenCalled();
  });
  ...
  test("can open create dialog", async () => {
    ...
    expect(screen.getByTestId("create-allowance-dialog")).toBeInTheDocument();
  });
});
