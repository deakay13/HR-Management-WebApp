...
export const calculatePayroll = async (req, res) => {
  try {
    ...
    const { MaBL, MaNV, MaLCB, MaPC, MaKT, MaGL, Thang, SoNgayLam } = parsed.data;
...
export const updatePayroll = async (req, res) => {
  try {
    const { ID } = req.params;
    const parsed = payRollSchema.omit({ MaBL: true }).safeParse(req.body);
    if (!parsed.success) {
      const errors = parsed.error.issues.map((issue) => ({
        field: issue.path[0],
        message: issue.message,
    ...
    const { MaLCB, MaPC, MaKT, MaGL, Thang, SoNgayLam } = parsed.data;
    ...
    const newSoNgayLam =
      SoNgayLam || payroll.SoNgayLam || gioLam.SoNgayLam || 26;
...
export const getPayrollByEmployee = async (req, res) => {
  try {
    ...
    if (isEmployee && ID !== req.account.MaNV) {
      return res
        .status(403)
        .json({ message: "Không có quyền xem lương của nhân viên khác" });
    }
...
export const searchPayroll = async (req, res) => {
  try {
    ...
    const result = await searchService(BangLuong, req.query, pagination, {
      searchFields: [
        "MaNV",
        "MaBL",
        "MaKT",
  ...
  try {
    ...
    for (const p of payrolls) {
      ...
      const row = worksheet.addRow({
        ...
        PhuCap: p.PhuCapThuong
          ? `${p.PhuCapThuong.LoaiPC} (${Number(p.PhuCapThuong.SoTien).toLocaleString("vi-VN")} VND)`
          : "0",
        KhauTru: p.KhauTru
          ? `${p.KhauTru.LoaiKT} (${p.KhauTru.PhanTram}%)`
          : "0",
